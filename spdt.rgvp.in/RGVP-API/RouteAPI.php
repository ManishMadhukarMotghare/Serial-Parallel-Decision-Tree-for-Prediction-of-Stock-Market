...
switch ($CommandType) {
    ...
    case "INSERT":
        ...
        if (isset($_REQUEST)) {
            ...
            header("location:../route-view.php?txt_id=".$id);
 ...
 header("location:../route-view.php?txt_id=".$id."&Msg=Your Data is Updated Succesfully");
    ...
    case "DELETE":
        ...
        if (isset($_GET["id"]) && 0 < intval($_GET["id"])) {
            //dbinit($gaSql);
            // REMOVE DATA
             $Sql_input = "update route set Status='Inactive' WHERE $sIndexColumn = " . intval($_GET["id"]);
    ...
    case "SELECT":
        ...
        while ($aRow = mysqli_fetch_array($rResult)) {
            ...
            $output["data"][] = array_merge(array('<a href="route-view.php?txt_id=' . $row[0] . '" class="btn btn-warning" ><i class="fa fa-eye"></i></a>'), $row);
        }
        // RETURN IN JSON

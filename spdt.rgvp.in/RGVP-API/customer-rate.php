...
$SelectColumns = array("CustomerRateID", "CustomerID", "RateType", "FromBranch", "ToBranch", "TransitTime", "RateSubType", "MinimumQuantity", "Rate", "Status", "AddedBy", "AddedDate", "UpdateBy", "UpdatedDate"); // array("id", "name", "email", "mobile", "start_date");
$InsertColumns = array("CustomerID", "RateType", "FromBranch", "ToBranch", "TransitTime", "RateSubType", "MinimumQuantity", "Rate", "Status", "AddedBy", "UpdateBy");
$UpdateColumns = array("CustomerID", "RateType", "FromBranch", "ToBranch", "TransitTime", "RateSubType", "MinimumQuantity", "Rate", "Status", "AddedBy", "UpdateBy");
$InsertColumnsData = array("txt_CustomerID", "ddl_RateType", "txt_FromBranch", "txt_ToBranch", "txt_TransitTime", "ddl_RateSubType", "txt_MinimumQuantity", "txt_Rate", "ddl_Status", "txt_AddedBy", "txt_UpdateBy");
$ColumnAPIUpdateData = array("txt_CustomerID", "ddl_RateType", "txt_FromBranch", "txt_ToBranch", "txt_TransitTime", "ddl_RateSubType", "txt_MinimumQuantity", "txt_Rate", "ddl_Status", "txt_AddedBy", "txt_UpdateBy");
$InsertColDataString = "";
$UpdateColDataString = "";
$sIndexColumn = "CustomerRateID";
$sTable = "customerrate";
...
switch ($CommandType) {
    ...
    case "DELETE":
        ...
        if (isset($_GET["id"]) && 0 < intval($_GET["id"])) {
            //dbinit($gaSql);
            // REMOVE DATA
            $Sql_input = "update customerrate set Status='Inactive' WHERE $sIndexColumn = " . intval($_GET["id"]);
    ...
    case "SELECT-WITH-ID":
        ...
        break;
    case "SELECT-WITH-ID-BRANCH":
        if (isset($_GET["CustomerID"]) && 0 < intval($_GET["CustomerID"])) {
            $SWhere = "WHERE FromBranch = '" . $posted["FromBranch"] . "' AND ToBranch = '" . $posted["ToBranch"] . "' AND CustomerID = " . $posted["CustomerID"];
            $Sql_input = "SELECT * FROM $sTable $SWhere;";
            $Sql_output = $RGVP->DB->Execute_Query($Sql_input, "GET", "SQLObj");
            die(json_encode(mysqli_fetch_assoc($Sql_output)));
        }
        break;
    ...
    case "SELECT":
        ...
        while ($aRow = mysqli_fetch_array($rResult)) {
            ...
            $output["data"][] = array_merge(array('<a data-id="row-' . $row[0] . '" href="javascript:editRowrate(' . $row[0] . ');" class="btn btn-success"><i class="fa fa-edit"></i></a>&nbsp;<a href="javascript:removeRowrate(' . $row[0] . ');" class="btn btn-danger" ><i class="fa fa-close"></i></a>'), $row);
        }
        // RETURN IN JSON

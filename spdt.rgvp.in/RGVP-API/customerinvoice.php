...
$SelectColumns = array("InvoiceID", "InvoiceNo", "InvoiceDate", "InvoiceBranch", "CustomerID", "CustomerName", "CustomerEmail", "CustomerAddress", "CustomerPhone", "GSTIN", "InvoiceStartDate", "InvoiceEndDate", "Dockets", "InvoiceDueDate", "InvoiceDueDateCharges", "PaidAmount", "BillingAmount", "GSTAmount", "TotalAmount", "InvoiceStatus", "AddedBy", "AddedDate", "UpdateBy", "UpdateDate"); // array("id", "name", "email", "mobile", "start_date");
...
$UpdateColumns = array("InvoiceNo", "InvoiceDate", "InvoiceBranch", "CustomerID", "CustomerName", "CustomerEmail", "CustomerAddress", "CustomerPhone", "GSTIN", "InvoiceStartDate", "InvoiceEndDate", "Dockets", "InvoiceDueDate", "InvoiceDueDateCharges", "PaidAmount", "BillingAmount", "GSTAmount", "TotalAmount", "InvoiceStatus", "AddedBy", "UpdateBy");
...
switch ($CommandType) {
        ...
        if (isset($_REQUEST)) {
          ...
          $Sql_input = "INSERT INTO $sTable (" . implode(",", $InsertColumns) . ") VALUES ($InsertColDataString)";
            ...
            $id = $RGVP->DB->PrimaryKeyID;
              $InvoiceNo = "AONE/" . $posted["txt_InvoiceBranch"] . "/" . $id . "";
            $Sql_input1 = "UPDATE customerinvoice Set InvoiceNo = '$InvoiceNo' where InvoiceID = $id ";
    ...
    case "SELECT":
        ...
        while ($aRow = mysqli_fetch_array($rResult)) {
            ...
            $output["data"][] = array_merge(array('<a data-id="row-' . $row[0] . '" href="javascript:editstatus(' . $row[0] . ');" class="btn btn-info"><i class="fa fa-check-square"></i></a><a data-id="row-' . $row[0] . '" href="javascript:editRow(' . $row[0] . ');" class="btn btn-warning"><i class="fa fa-eye"></i></a><a href="view-customerinvoice.php?txt_InvoiceID=' . $row[0] . '" target="_blank"  class="btn btn-success"><i class="fa fa-print"></i></a><a href="javascript:removeRow(' . $row[0] . ');" class="btn btn-danger" ><i class="fa fa-close"></i></a>'), $row);
        }
        // RETURN IN JSON

...
$SelectColumns = array("GSTID", "GstRate", "CGST", "SGST", "IGST", "CESS", "Status", "AddedBy", "AddedDate", "UpdateBy", "UpdateDate"); // array("id", "name", "email", "mobile", "start_date");
$InsertColumns = array("GstRate", "CGST", "SGST", "IGST", "CESS", "Status", "AddedBy", "UpdateBy");
$UpdateColumns = array("GstRate", "CGST", "SGST", "IGST", "CESS", "Status", "AddedBy", "UpdateBy");
$InsertColumnsData = array("txt_GstRate", "txt_CGST", "txt_SGST", "txt_IGST", "txt_CESS", "ddl_Status", "txt_AddedBy", "txt_UpdateBy");
$ColumnAPIUpdateData = array("txt_GstRate", "txt_CGST", "txt_SGST", "txt_IGST", "txt_CESS", "ddl_Status", "txt_AddedBy", "txt_UpdateBy");
$InsertColDataString = "";
$UpdateColDataString = "";
$sIndexColumn = "GSTID";
$sTable = "gstrate";
...
switch ($CommandType) {
    ...
    case "INSERT":
        ...
        if (isset($_REQUEST)) {
            ...
            header("location:../gstrate-view.php?txt_id=" . $id);
...
//            die(json_encode(mysqli_fetch_assoc($Sql_output)));
            header("location:../gstrate-view.php?txt_id=" . $id . "&Msg=Your Data is Updated Succesfully");
        }
        break;
    case "DELETE":
        ...
        if (isset($_GET["id"]) && 0 < intval($_GET["id"])) {
            //dbinit($gaSql);
            // REMOVE DATA
            $Sql_input = "update gstrate set Status='Inactive' WHERE $sIndexColumn = " . intval($_GET["id"]);
    ...
    case "SELECT":
        ...
        while ($aRow = mysqli_fetch_array($rResult)) {
            ...
            for ($i = 0; $i < count($SelectColumns); $i++) {
             ...
             $output["data"][] = array_merge(array('<a href="gstrate-view.php?txt_id=' . $row[0] . '" class="btn btn-warning" ><i class="fa fa-eye"></i></a>'), $row);
        }
        // RETURN IN JSON

<?php include "include.php";// VARIABLES
                        $SelectColumns = array("DepartmentID","Name","Alias","LogoImage","Intro","Discription","BannerImage","Order","Status","AddedBy","UpdateBy","AddedDate","UpdateDate"); // array("id", "name", "email", "mobile", "start_date");
            ...
            $posted["txt_BannerImage"] = "";
                                ...
                                $Sql_input = "INSERT INTO $sTable (".implode(",", $InsertColumns).") VALUES ($InsertColDataString)";
                                    ...
                                    $id = $RGVP->DB->PrimaryKeyID;
                                     ...
                                     $path = $_SERVER['DOCUMENT_ROOT'] . "/images/department/" . $id . "/";
            if (!is_dir($path))
     ...
     if ($FileUploadStatus && $FileUploadStatus1) {
              $Sql_input = "UPDATE Department SET LogoImage = '" . $logo_name . "' ,  BannerImage = '" . $banner_name . "' where DepartmentID = '$id'";
                        ...
                        // AJAX REMOVE FROM JQUERY
                            ...
                            $sQuery = " SELECT ".implode(",$sTable.",$SelectColumns)." FROM $sTable $sWhere $sOrder $sLimit ";
                                ...
                                while ($aRow = mysqli_fetch_array($rResult)) {
                                    ...
                                    $output["data"][] = array_merge(array('<a href="view-Department.php?ID=' . $row[0] . '" class="btn btn-warning" ><i class="fa fa-eye"></i></a>&nbsp;<a href="javascript:removeRow(' . $row[0] . ');" class="btn btn-danger" ><i class="fa fa-close"></i></a>'),$row, $row);
                                }
                                // RETURN IN JSON

...
namespace RGVPCore {
    ...
    Class RGVPCore {
        ...
        static function GETStatus($Code, $status = TRUE, $AdditionalInfo = NULL, $Data = NULL) {
            ...
            return '{"status": "' . strtolower($StatusText) . '", "message":"' . $Response . '","result":{"statuscode":"' . $Code . '","AdditionalInfo":"' . $AdditionalInfo . '","HtmlResponse":"' . self::ReportMsg($AdditionalInfo, $StatusText, $Code . " : " . $Response) . '","Data": "' . $Data . '"}}';
        }
    ...
    Class RGVP_Cryptography {
        ...
        public static function getToken($length, $CharSet = "DigitsDEC") {
            $token = "";
            $codeAlphabet = "";
            ...
            if ($To == "") {
                ...
                if ($SMSResponse != "Error")
                    if (is_numeric($SMSResponse))
                        return RGVP_StatusCodes::GETStatus("650", TRUE, "SMS Sent Successfully.", '{"Response":"' . $SMSResponse . '","ErrorCatch":"' . $ErrorResponse . '"}');
                    else
                        return RGVP_StatusCodes::GETStatus("653", TRUE, "SMS Not Sent. SMSError.", '{"Response":"' . $SMSResponse . '","ErrorCatch":"' . $ErrorResponse . '"}');
                else
                    return RGVP_StatusCodes::GETStatus("651", TRUE, "SMS Not Sent. CatchError.", '{"Response":"' . $SMSResponse . '","ErrorCatch":"' . $ErrorResponse . '"}');
            }
        }
...
//$amount = '10000034000';
    ...
    class RGVPDB {
         ...
         * 
         * If(Error is Occured)
         * @return SQLiObject => if connection is sucessful.
         *  
         */
        static function RGVPDB_GetConnection($DBHost = NULL, $DBUser = NULL, $DBPassword = NULL, $DBName = NULL) {
            if ($DBHost == NULL || $DBUser == NULL || $DBPassword == NULL)
                return mysqli_connect($GLOBALS["RGVPDB_host"], $GLOBALS["RGVPDB_user"], $GLOBALS["RGVPDB_pass"], $GLOBALS["RGVPDB_name"]);
            else
            ...
            if ($this->RGVPDB_Connect()) {
                ...
                if ($RGVPQueryResult === FALSE) {
                    ...
                    if ($CommandType == "SET") {
                        ...
                        $this->Response = RGVP_StatusCodes::GETStatus($this->StatusCode, TRUE, $this->Msg, $this->PrimaryKeyID);
                        ...
                        $this->Response = RGVP_StatusCodes::GETStatus($this->StatusCode, TRUE, $this->Msg, $PrimaryKeyID);
                        return $this->Response;
                    }
                }
            } else {
...
//["flags"]=> string(44) "NOT_NULL PRI_KEY AUTO_INCREMENT NUM PART_KEY" ["type"]=> string(4) "LONG" ["decimals"]=> int(0) 
    ...
    static function ExecuteQuerySELECT($RGVPQuery, $RGVPQueryReturn = "SET") {
        ...
        function FillSelectQuery($RGVPFillQuery, $DisplayColumn, $ValueColumn = NULL, $ControlID = NULL, $ControlName = NULL, $CssClass = "form-control") {
